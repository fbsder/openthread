...
static inline void _spiRXCheckAndReceive(struct spi_data *spi_data)
{
	uint32_t data;
...
static inline void _spiTXCheckAndSend(struct spi_data *spi_data)
{
	uint32_t data;
	SercomSpi *spi = (SercomSpi *)SERCOM4;
...
static inline int32_t _spiErrorCheck(void)
{
	SercomSpi *spi = (SercomSpi *)SERCOM4;
...
int samr21SpiTransceive(const void *tx_buf, uint32_t tx_buf_len,
                              void *rx_buf, uint32_t rx_buf_len)
{
	...
	{
		_spiTXCheckAndSend(local_data);

		_spiRXCheckAndReceive(local_data);
